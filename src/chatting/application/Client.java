...
import java.io.*;
 ...
 public class Client  // extends JFrame/*this JFrame class is used for making frame */
        ...
        static JPanel a1;// globally decalring so the value get append over the JPanel a1
  ...
  Client (){
        ...
        p1.setBackground(new Color(255,215,0 ));//giving color to the panel made over frame
    ...
    //user can write their message
       ...
       send.setBackground(new Color(255,215,0));//Without this default 'send' button will be shown 
       ...
       //frame
        ...
        f.setLocation(800,50);//set location of frame otherwise willbe shown in right corner
...
public void actionPerformed(ActionEvent ae){
    //putting it in try and catch beacuuse we are working with the server 
    try{
   ...
   // p2.add(output);
    ...
    dout.writeUTF(out);//for sending message to the server
...
return panel;
    ...
    public static void main(String[] args){
        ...
        try{
            //making socket and connecting it with server
            Socket s= new Socket("127.0.01",6001);//first given the local host address then the address of server is given ie 6001
            DataInputStream din= new DataInputStream(s.getInputStream());//for receiving messages
                 ...
                 while(true){
                    ...
                    a1.setLayout(new BorderLayout());
                    // now the read msg is to shown,coming from server end
